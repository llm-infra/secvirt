...
import (
	...
	context "context"
	errors "errors"
	filesystem "github.com/llm-infra/secvirt/sdk-go/sandbox/spec/filesystem"
	http "net/http"
	strings "strings"
)
...
// period.
const (
	// FilesystemReadProcedure is the fully-qualified name of the Filesystem's Read RPC.
	FilesystemReadProcedure = "/filesystem.Filesystem/Read"
	// FilesystemWriteProcedure is the fully-qualified name of the Filesystem's Write RPC.
	FilesystemWriteProcedure = "/filesystem.Filesystem/Write"
...
type FilesystemClient interface {
	Read(context.Context, *connect.Request[filesystem.ReadRequest]) (*connect.ServerStreamForClient[filesystem.ReadResponse], error)
	Write(context.Context) *connect.ClientStreamForClient[filesystem.WriteRequest, filesystem.WriteResponse]
...
func NewFilesystemClient(httpClient connect.HTTPClient, baseURL string, opts ...connect.ClientOption) FilesystemClient {
	...
	return &filesystemClient{
		read: connect.NewClient[filesystem.ReadRequest, filesystem.ReadResponse](
			httpClient,
			baseURL+FilesystemReadProcedure,
			connect.WithSchema(filesystemMethods.ByName("Read")),
			connect.WithClientOptions(opts...),
		),
		write: connect.NewClient[filesystem.WriteRequest, filesystem.WriteResponse](
			httpClient,
			baseURL+FilesystemWriteProcedure,
			connect.WithSchema(filesystemMethods.ByName("Write")),
			connect.WithClientOptions(opts...),
		),
...
type filesystemClient struct {
	read             *connect.Client[filesystem.ReadRequest, filesystem.ReadResponse]
	write            *connect.Client[filesystem.WriteRequest, filesystem.WriteResponse]
...
// Read calls filesystem.Filesystem.Read.
func (c *filesystemClient) Read(ctx context.Context, req *connect.Request[filesystem.ReadRequest]) (*connect.ServerStreamForClient[filesystem.ReadResponse], error) {
	return c.read.CallServerStream(ctx, req)
}

// Write calls filesystem.Filesystem.Write.
func (c *filesystemClient) Write(ctx context.Context) *connect.ClientStreamForClient[filesystem.WriteRequest, filesystem.WriteResponse] {
...
type FilesystemHandler interface {
	Read(context.Context, *connect.Request[filesystem.ReadRequest], *connect.ServerStream[filesystem.ReadResponse]) error
	Write(context.Context, *connect.ClientStream[filesystem.WriteRequest]) (*connect.Response[filesystem.WriteResponse], error)
	Stat(context.Context, *connect.Request[filesystem.StatRequest]) (*connect.Response[filesystem.StatResponse], error)
	Move(context.Context, *connect.Request[filesystem.MoveRequest]) (*connect.Response[filesystem.MoveResponse], error)
	Remove(context.Context, *connect.Request[filesystem.RemoveRequest]) (*connect.Response[filesystem.RemoveResponse], error)
	MakeDir(context.Context, *connect.Request[filesystem.MakeDirRequest]) (*connect.Response[filesystem.MakeDirResponse], error)
...
func NewFilesystemHandler(svc FilesystemHandler, opts ...connect.HandlerOption) (string, http.Handler) {
	filesystemMethods := filesystem.File_filesystem_filesystem_proto.Services().ByName("Filesystem").Methods()
	filesystemReadHandler := connect.NewServerStreamHandler(
		FilesystemReadProcedure,
		svc.Read,
		connect.WithSchema(filesystemMethods.ByName("Read")),
		connect.WithHandlerOptions(opts...),
	)
	filesystemWriteHandler := connect.NewClientStreamHandler(
		FilesystemWriteProcedure,
		svc.Write,
		connect.WithSchema(filesystemMethods.ByName("Write")),
		connect.WithHandlerOptions(opts...),
	)
	...
	return "/filesystem.Filesystem/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case FilesystemReadProcedure:
			filesystemReadHandler.ServeHTTP(w, r)
		case FilesystemWriteProcedure:
			filesystemWriteHandler.ServeHTTP(w, r)
...
func (UnimplementedFilesystemHandler) Read(context.Context, *connect.Request[filesystem.ReadRequest], *connect.ServerStream[filesystem.ReadResponse]) error {
	return connect.NewError(connect.CodeUnimplemented, errors.New("filesystem.Filesystem.Read is not implemented"))
}

func (UnimplementedFilesystemHandler) Write(context.Context, *connect.ClientStream[filesystem.WriteRequest]) (*connect.Response[filesystem.WriteResponse], error) {
	return nil, connect.NewError(connect.CodeUnimplemented, errors.New("filesystem.Filesystem.Write is not implemented"))
}
